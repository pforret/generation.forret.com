...
`<generation>` = one of: `lost-generation`, `interbellum-generation`, `greatest-generation`,
`silent-generation`, `baby-boomers`, `generation-x`, `millennials`, `generation-z`,
`generation-alpha`, `generation-beta`.

| Route | Purpose | Anchor |
|---|---|---|
| `/` | Home / overview hub (data table of all generations) | — |
| `/generations/` | Hub: all generations + comparison table | — |
| `/generations/<generation>/` | Generation page (summary, years, traits, FAQ) | — |
| `/generations/<generation>/famous-people/` | Notable people overview ("Best X") | born‑in |
| `/generations/<generation>/actors/` | Actors born in this generation | born‑in |
| `/generations/<generation>/musicians/` | Solo musicians born in this generation | born‑in |
| `/generations/<generation>/politicians/` | Politicians born in this generation | born‑in |
| `/generations/<generation>/business-leaders/` | Business people born in this generation | born‑in |
| `/generations/<generation>/quotes/` | Quotes from this generation | — |
| `/generations/<generation>/timeline/` | Defining events during its life stages | — |
| `/generations/<generation>/influences/` | Bands/movies/TV that peaked during its youth | peaked‑during |
| `/compare/` | Hub: comparison matrix grid | — |
| `/compare/<older>-vs-<newer>/` | All 45 ordered pairs (incl. skip‑level); reverse 301s to canonical | — |
| `/events/` | Hub: events by era/decade | — |
| `/events/<event>/` | Event page + per‑generation influence | peaked‑during |
| `/people/` | A–Z hub of individuals | — |
| `/people/<person>/` | Person page | born‑in |
| `/bands/` · `/bands/<band>/` | Music groups | peaked‑during |
| `/movies/` · `/movies/<movie>/` | Films | peaked‑during |
| `/tv-shows/` · `/tv-shows/<show>/` | TV programs | peaked‑during |
| `/quotes/` | Hub: all quotes, grouped by generation | — |
| `/what-generation-am-i/` | Birth‑year lookup + explainer | — |
| `/born-in/` | Hub: all years, grouped by decade | — |
| `/born-in/<decade>s/` | Decade index (e.g. `/born-in/1980s/`) | — |
| `/born-in/<year>/` | One page per year 1883–2025 | — |
| `/glossary/` · `/glossary/<term>/` | Term definitions | — |
| `/blog/` | Articles hub | — |
| `/blog/<article-slug>/` | Free‑form article | — |
| `/blog/best-of-<generation>/` | Top 5 most positively influential (10 posts) | born‑in |
| `/blog/worst-of-<generation>/` | Top 5 most negatively influential (10 posts) | born‑in |
| `/blog/best-<category>-of-<generation>/` | Optional category cut | born‑in |
| `/about/` | About / methodology / sources | — |
| `/sitemap.xml` | Auto‑generated by MkDocs | — |
| `/robots.txt` | Static — allow AI crawlers, link sitemap | — |

**Category lists extensible per `Person.category`:** `/generations/<gen>/athletes/`,
`/writers/`, `/scientists/`, … (same born‑in anchor).

**Redirects (old Laravel → new):** `/generation` → `/generations/`,
`/generation/{slug}` → `/generations/{slug}/`, `/person` → `/people/`,
`/person/{slug}` → `/people/{slug}/`, plus slug aliases (`gen-x` → `generation-x`,
`generation-y` → `millennials`, etc. — see the slug table above).

**Approximate page count:** ~10 generations × ~10 cluster pages (~100) + 45 compare +
events/bands/movies/tv (100s) + people (100s) + ~143 born‑in years + decade indexes +
glossary + ~20 blog‑series posts + hubs. The bulk is data‑generated, not hand‑authored.
